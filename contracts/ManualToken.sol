...
contract TokenERC20 {
   ...
   *
   * Initialises contract with initials supply tokens to the creator of the contract
   */
  constructor(
    uint256 initialSupply,
    string memory FinArtToken,
    string memory FINAI
  ) {
    totalSupply = initialSupply * 10 ** uint256(decimals); // Update total supply with the decimal amount
    balanceOf[msg.sender] = totalSupply; // Give the creator all initial tokens
    name = FinArtToken; // Set the name for display purposes
    symbol = FINAI; // Set the symbol for display purposes
  }
